...
const handlePhotoFileChange = async (event: Event) => {
  ...
  if (input.files && input.files.length > 0) {
    ...
    // 批量选择：能收多少收多少 —— 不合格式的跳过，超出数量的截掉，最后只弹一条提示
    const kept: File[] = [];
    let firstError = '';
    ...
    const capacity = Math.max(0, maxPhotos - uploadedPhotoImages.value.length);
    const overflow = kept.length > capacity;
    files = kept.slice(0, capacity);
...
const handleVideoRefUpload = async (event: Event) => {
  ...
  if (input.files && input.files.length > 0) {
    ...
    // 数量先于格式：这一批只要数量超了就整批不收，提示也以数量为准。
    // 视频 tab 的参考文件不做部分收下 —— 少一个文件出来的效果和用户预期差太远。
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        let duration = fileType === 'video' || fileType === 'audio' ? await getMediaDuration(file) : 0;
        ...
        if (fileType === 'video' && uploadedUrl && duration > refVideoMaxSeconds.value) {
        ...
        if (uploadedUrl) {
          ...
          if (fileType === 'video' && selectedVideoMultimodal.value === 'multimodal' && videoLimitMode.value === 'unlimited') {
