...
// Flag to skip reset when switching from history to local during publish
let isPublishingFromHistory = false;

// Watch for upload option changes
watch(uploadOption, (newOption, oldOption) => {
  console.log('[DEBUG] uploadOption changed from', oldOption, 'to', newOption);
  console.log('[DEBUG] Before change - selectedCollection:', selectedCollection.value, 'isNoCollection:', isNoCollection.value);
  console.log('[DEBUG] isPublishingFromHistory:', isPublishingFromHistory);

  // Skip reset if we're publishing from history (already set collection)
  if (isPublishingFromHistory && oldOption === 'history') {
    console.log('[DEBUG] Skipping reset - publishing from history');
    isPublishingFromHistory = false;
    return;
  }

  // Make sure the flag is reset for other cases
  isPublishingFromHistory = false;

  if (newOption == 'history') {
    console.log('[DEBUG] Switching to history mode');
    fetchProjects();
  } else if (newOption == 'local') {
    console.log('[DEBUG] Switching to local mode');
    // Reset selected project when switching to local upload mode
    selectedProject.value = null;
    selectedProjectId.value = '';
    selectedEpisode.value = null;
    // Reset collection to no collection for local upload mode
    // User can manually select a collection if needed
    selectedCollection.value = '';
    selectedCollectionId.value = '';
    selectedEpisodeNumber.value = '1';
    isNoCollection.value = true;
    episodes.value = [{ value: '1', label: t('submit.image.episode', { episode: 1 }) }];
    console.log('[DEBUG] After resetting for local mode - selectedCollection:', selectedCollection.value, 'isNoCollection:', isNoCollection.value);
  }
});
...
const canSubmit = computed(() => {
  return imageFiles.value.length > 0 && selectedCollection.value && coverPreview.value;
});
    ...
    fetch(baseUrl + "user/uploadImage", parma)
      ...
      .then((res: any) => {
        if (res.code === 0 || res.code === 200) {
          ...
          console.log('[DEBUG] Image upload successful - selectedCollection:', selectedCollection.value, 'isNoCollection:', isNoCollection.value, 'uploadOption:', uploadOption.value);
...
function onCoverConfirmed(coverUrl: string) {
  ...
  // Set cover preview directly since SetImageCoverModal has already uploaded the image
...
async function onSubmit() {
  ...
  if (!selectedCollection.value) {
    toast(t("submit.collectionRequired"));
    return;
  }
...
async function selectProject(project: any) {
  console.log('[DEBUG] selectProject called with project:', project?.name, 'project.id:', project?.id);
  ...
  if (projectDetailsCache.value[project.session_id]) {
    ...
    if (project.name) {
      console.log('[DEBUG] selectProject (cached): project.name exists, calling handleCollectionFromProjectName');
      await handleCollectionFromProjectName(project.name);
    } else {
      console.log('[DEBUG] selectProject (cached): project.name is empty, resetting collection');
    ...
    console.log('[DEBUG] selectProject (cached) - final state: selectedCollection:', selectedCollection.value, 'isNoCollection:', isNoCollection.value);
    return;
  }
  ...
  try {
    ...
    if (res.code == 200 && res.data) {
      ...
      // Handle collection logic for history projects
      if (project.name) {
        console.log('[DEBUG] selectProject (API): project.name exists, calling handleCollectionFromProjectName');
        await handleCollectionFromProjectName(project.name);
      } else {
        console.log('[DEBUG] selectProject (API): project.name is empty, resetting collection');
        // Reset collection state for projects without names
        selectedCollection.value = '';
        selectedCollectionId.value = '';
        isNoCollection.value = true;
      ...
      console.log('[DEBUG] selectProject (API) - final state: selectedCollection:', selectedCollection.value, 'isNoCollection:', isNoCollection.value);
    }
  } catch (error) {
...
async function handleCollectionFromProjectName(projectName: string) {
  console.log('[DEBUG] handleCollectionFromProjectName called with projectName:', projectName);
  try {
    // Search for collection by title
    const searchRes = await api.searchCollection({ title: projectName, type: 1 }) as any;
    console.log('[DEBUG] searchCollection result:', searchRes);
...
async function handlePublish(publishData?: any) {
  ...
  // 切换到本地上传标签页，显示有图片的块
  console.log('[DEBUG] handlePublish: setting isPublishingFromHistory = true');
  isPublishingFromHistory = true;
...
onMounted(async () => {
  ...
  if (postId.value) {
    ...
    if (session_id && index) {
      console.log('[DEBUG] onMounted: session_id and index found', 'title:', title);
      // Switch to local upload mode since we're loading generated content
      uploadOption.value = 'local';

      // Reset selected project when loading generated content
      selectedProject.value = null;
      selectedProjectId.value = '';
      selectedEpisode.value = null;
      ...
      try {
        ...
        if (chapterResponse.code === 200 && chapterResponse.data) {
          ...
          // NOTE: When loading generated content (session_id + index), we don't auto-set collection
          // User can manually select a collection if needed
          console.log('[DEBUG] onMounted: session_id and index found, not auto-setting collection even if title exists');
          selectedCollection.value = '';
          selectedCollectionId.value = '';
          selectedEpisodeNumber.value = '1';
          isNoCollection.value = true;
