...
const imgScale = ref(0.01); // Start with very small scale to prevent flash
...
const imageStyle = computed(() => {
  return {
    transform: `translate(-50%, -50%) translate(${imgOffsetX.value}px, ${imgOffsetY.value}px) scale(${imgScale.value})`,
  };
});
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      imgOffsetY.value = 0;
      imgOffsetX.value = 0;
      imgScale.value = 1; // Start with normal scale
      localImage.value = null;

      // Check if cover is canvas-generated (not from uploaded images)
      if (props.isCanvasGenerated) {
        // For canvas-generated covers, still show cover image initially but allow user to change
        if (props.coverImage) {
...
function confirm() {
  ...
  cropToCanvas(src).then((cropped) => {
    ...
    // If crop fails (cross-origin issue), use the original URL directly without cropping
    toast("图片来自外部网站，将直接使用原图作为封面");
    emit("confirm", src);
    close();
  });
}
...
async function detectOrientation(dataUrl: string) {
  ...
  const { width: CROP_W, height: CROP_H } = cropDimensions.value;
  const targetRatio = CROP_W / CROP_H; // 3:4
  const imageRatio = img.naturalWidth / img.naturalHeight;

  let scale;
  if (imageRatio >= targetRatio) {
    // 横图或正方形 - 按高度缩放以确保高度足够
    scale = CROP_H / img.naturalHeight;
  } else {
    // 竖图 - 按宽度缩放
    scale = CROP_W / img.naturalWidth;
  }
  ...
  // 计算居中偏移：负数表示向左/向上移动
  // 横图：宽度超出裁剪框，需要向左移动使其水平居中
  // 竖图：高度超出裁剪框，需要向上移动使其垂直居中
  const centerOffsetX = (CROP_W - scaledWidth) / 2;
  const centerOffsetY = (CROP_H - scaledHeight) / 2;

  // 设置偏移，确保图片居中显示
  imgOffsetX.value = centerOffsetX;
  imgOffsetY.value = centerOffsetY;
...
async function cropToCanvas(dataUrl: string): Promise<string> {
  ...
  if (!dataUrl.startsWith('data:')) {
    try {
      base64Data = await urlToBase64(dataUrl);
    } catch (e) {
      throw new Error("无法加载跨域图片，请下载图片后重新上传");
    }
  }
